  ...
  return around(tape)
    .before((t) => {
      const mongoStore = {
        ...
        client: {
          get: sinon.stub().resolves(clientData),
        },
        beer: {
          get: sinon.stub().resolves(beerData),
        },
        keg: {
          get: sinon.stub().resolves(kegData),
        },
      };
      const cloudStore = {
        tap: {
...
createTest(tapSetupMode, client, beer, null)('throws ValidationError if keg entity is missing', async (t, mongoStore, cloudStore, setupData) => {
  const interactor = new SetupTap(mongoStore, cloudStore);
