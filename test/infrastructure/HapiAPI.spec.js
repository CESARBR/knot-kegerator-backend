...
const test = around(tape)
  .before((t) => {
    ...
    const tapService = {
      ...
      listTaps: sinon.stub().resolves(tapsData),
    };
    const beerService = {
...
test('listTaps() calls TapService.listTaps()', async (t, hapiAPI) => {
  await hapiAPI.listTaps();

  t.true(hapiAPI.tapService.listTaps.called);
  t.end();
});

test(
  'listTaps() returns a list of taps returned by TapService.listTaps()',
  async (t, hapiAPI) => {
    const expectedTaps = [
      {
        id: '7bfe7203-2617-4590-bfac-8d48923fbf01',
    ...
    const actualTaps = await hapiAPI.listTaps();
