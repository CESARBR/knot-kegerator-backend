...
test('listKegs() calls ListKegs.execute()', async (t, kegService) => {
  await kegService.listKegs();

  t.true(kegService.listKegsInteractor.execute.called);
  t.end();
});

test('listKegs() returns Kegs returned by ListKegs.execute()', async (t, kegService) => {
  const expectedKegs = [
    {
      id: 'd6600558-f101-45be-bf8a-4b5aed40cf9f',
      name: 'Stainless steel',
  ...
  const actualKegs = await kegService.listKegs();
