...
test('list() calls ListBeers.execute()', async (t, beerService) => {
  await beerService.list();

  t.true(beerService.listBeersInteractor.execute.called);
  t.end();
});

test('list() returns Beers returned by ListBeers.execute()', async (t, beerService) => {
  const expectedBeers = [
    {
      id: '1fb78cbb-5fc1-46fd-80e5-cf541b905324',
      name: 'Capunga American Pale Ale',
  ...
  const actualBeers = await beerService.list();
