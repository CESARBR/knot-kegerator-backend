...
test('setup() calls SetupTap.execute()', async (t, tapService) => {
  ...
  await tapService.setup(request);

  t.true(tapService.setupTapInteractor.execute.called);
  t.end();
});

test('setup() calls SetupTap.execute() with request', async (t, tapService) => {
  const request = new SetupTapRequest(
    '9392780a-1d43-47a8-ab05-b425eeee96f4',
    'a42e08c1-d278-444d-9451-72f70d916c61',
    'b60b53df-caab-41a6-9878-c00e23e504ab',
...
test('setup() validates request', async (t, tapService) => {
  try {
    await tapService.setup();
    t.fail('should throw');
  } catch (e) {
    t.pass('should throw');
  }
